...
int HashTableBuildDestroyTest ()
{
	...
	if (HashTableDestruct (0) != -1) return TEST_DELETE;
...
int HashTableFindTest ()
{
	...
	char* test_key   = "keykey";
	char* test_data  = "data";
	char* find_str   = "keykey";
	char* no_find_str = "alligator";
	char* no_find_key = "no_key";
	...
	struct HashTable* TinyTable = HashTableBuild (MurmurHash2, 1);

	if (!TinyTable)
		return 0;


	if (HashTableInsert (TinyTable, no_find_key, test_data) == -1)
	{
		HashTableDestruct(TinyTable);
		return 0;
	}

	if (HashTableInsert (TinyTable, test_key, test_data) == -1)
	{
		HashTableDestruct(TinyTable);
		return 0;
	}
